    ...
    <div className="bg-gray-800 text-white p-0 flex flex-row items-center justify-between px-4 bg-opacity-85 text-5xl font-thin fixed bottom-0 w-full -mt-6 border-0">
      ...
      <>
        {weatherData.condition && (
          <img src={`https:${weatherData.condition.icon}`} alt={weatherData.condition.text} className="w-24 h-24 p-0 -m-2 mr-2" />
        )}
        {weatherData.temp_c && (
...
function PinIcon() {
  return (
    <svg className={`w-16 h-16 text-${globalconfig.accentColor}-400 `} aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
